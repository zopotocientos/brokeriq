...
export default function Groups() {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('groups')
      .select('id, name, zip, county, effective_date, sic_code, contact_name, contact_email, created_at, regions(id, region_number, region_name)')
  ...
  const filtered = groups
    .filter(g => {
      ...
      return g.name?.toLowerCase().includes(q) || g.county?.toLowerCase().includes(q) || g.zip?.includes(q);
    })
    .sort((a, b) => {
      let av, bv;
      if (sortField === 'region') {
        av = a.regions?.region_number ?? 99;
        bv = b.regions?.region_number ?? 99;
      } else {
                ...
                {filtered.map((g, i) => (
                  ...
                  >
                    <td style={{ padding: '0.875rem 1rem' }}>
                      <div style={{ fontWeight: 600, fontSize: '0.9375rem' }}>{g.name}</div>
                      {g.contact_name && <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '1px' }}>{g.contact_name}</div>}
                    </td>
                    <td style={{ padding: '0.875rem 1rem', fontSize: '0.9rem', fontFamily: 'monospace', color: 'var(--text-secondary)' }}>{g.zip}</td>
                    ...
                    <td style={{ padding: '0.875rem 1rem' }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', gap: '0.375rem' }}>
                        <ActionBtn title="Edit" onClick={() => openEdit(g)} icon="??" />
                        <ActionBtn title="Delete" onClick={() => setDeleteTarget(g)} icon="???" danger />
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 1.5rem', color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.5 }}>
              This will permanently delete the group and all associated census data and quotes. This cannot be undone.
            </p>
